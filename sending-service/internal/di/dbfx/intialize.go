	...
	lifecycle.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			...
			if err := client.Connect(ctx); err != nil {
				log.Error().Msgf("Failed to connect mongo DB due to %s", err.Error())
				return err
			}
			if err := client.Ping(ctx, nil); err != nil {
				log.Error().Msgf("Failed to ping mongo DB due to %s", err.Error())
				return err
			}
			log.Info().Msg("connect mongo DB successful!!!")
			return nil
		},
